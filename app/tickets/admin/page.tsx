...
export default function AdminTicketPage() {
  const [issuedTickets, setIssuedTickets] = useState<number[]>([]); // 💡 本当に存在する発行済み番号のリスト
  ...
  useEffect(() => {
    ...
    // 📢 本当に存在するチケットリストをリアルタイム受信
    onValue(ref(db, "issued_tickets"), (snapshot) => {
      const data = snapshot.val();
      if (data) {
        // オブジェクトのキー（番号）を取り出して数値配列にする
  ...
  // ❌ 発券取消（完全に番号の存在自体を消滅させる）
  const executeUndoIssue = async (num: number) => {
    if (calledNumbers.includes(num)) return alert("呼び出し中の番号は発券取り消しできません。先に呼び出しを取り消してください。");
    
    if (confirm(`「${num}番」の発券を取り消します。この番号のお客様画面は「未発券」に戻り、リストからも完全に消滅します。よろしいですか？`)) {
      // 💡 データベースからこの番号の存在を完全に消去（ズレを100%防止）
  ...
  const handleNextCall = async () => {
    // 💡 まだ呼び出されていない、存在する一番若い番号を自動で探して呼び出す
